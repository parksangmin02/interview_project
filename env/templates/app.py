...
def analyze_answer():
    ...
    prompt = f"""
당신은 면접 평가 전문가입니다.
다음 질문과 답변을 보고 아래 기준을 모두 분석하세요.

1) 아래 5개 항목을 0~100 정수 점수로 평가
   - 직무적합도(job_fit)
   - 논리성(logic)
   - 태도(attitude)
   - 구체성(specificity)
...
2) 답변의 **강점 2개**, **약점 2개**를 제시

3) ★ 답변의 **장점(pros)** 과 **단점(cons)** 를 문장 형태로 상세하게 2개씩 제시
...
"""
    ...
    try:
        ...
        return jsonify({
            ...
            "pros": parsed.get("pros", []),     # ★ 추가됨
            "cons": parsed.get("cons", [])      # ★ 추가됨
        }), 200
